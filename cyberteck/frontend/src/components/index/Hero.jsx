...
export default function Hero() {
    ...
    const [cardOffer, setCardOffer] = useState(cardOfferolde);

    const fetchCardOffer = async () => {
        const response = await axiosClient.get("/api/products");
        const data = response.data;
        console.log(data.products);
        setCardOffer(data.products);
    };
    useEffect(() => {
        fetchCardOffer();
    }, []);
    ...
    return (
        <section className="w-full h-auto flex flex-col items-center justify-center relative lg:px-24 md:px-20 px-6 mb-20">
            <div className="mt- w-full flex justify-between gap-5 items-center md:px-6 px-3">
                        ...
                        <div key={index} className="md:px-6 px-3">
                            ...
                            >
                                <div className="flex justify-between items-center">
                                    ...
                                    <Text
                                        as="small"
                                        className=" text-color3 font-semibold text-sm"
                                    >
                                        {"$"+cardOffer.price}
                                    </Text>
                                </div>
                                <div className="w-full flex gap-4 items-center justify-between text-color3">
                                    <div className="flex gap-4 justify-between">
                                        ...
                                        >
                                            {cardOffer.categories &&  cardOffer.categories.category_name}
                                        </Text>
                                    </div>
